...
pins the `Detector`/`Env`/`OnTuiModeSwitched` wiring. The queries are delayed until
150 ms after the terminal enters raw mode, so the first frame paints before any
theme change can rebuild; a theme change is panic-guarded and can never take the
TUI down. On a **committed** theme
