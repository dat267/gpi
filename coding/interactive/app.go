...
func (a *App) rebuildForTheme() {
	...
	// A theme rebuild must never take the TUI down: if it fails, report and keep
	// the current screen rather than propagating into the crash handler.
	defer func() {
		if r := recover(); r != nil {
			if a.options.WriteErr != nil {
				a.options.WriteErr(fmt.Sprintf("pier: theme rebuild failed: %v\n", r))
			}
		}
	}()
	if a.Startup != nil {
