...
// RecordBashResult records a bash execution result for the next prompt.
func (s *AgentSession) RecordBashResult(command string, result BashResult, excludeFromContext bool) {
	...
	s.Sessions.AppendCustomMessageEntry(RoleBashExecution, string(message.Content), false, nil)
	if excludeFromContext {
		return
	}
	if s.control != nil {
		s.control.stateMu.Lock()
		s.control.pendingBash++
		s.control.stateMu.Unlock()
	}
}
...
// HasPendingBashMessages reports whether recorded bash output awaits the next
// prompt.
func (s *AgentSession) HasPendingBashMessages() bool {
	...
	s.control.stateMu.Lock()
	defer s.control.stateMu.Unlock()
	return s.control.pendingBash > 0
}

// IsBashRunning reports whether a bash command is executing.
func (s *AgentSession) IsBashRunning() bool {
	...
	s.control.stateMu.Lock()
	defer s.control.stateMu.Unlock()
	return s.control.bashActive
}

// SetBashRunning records the bash execution state (host wiring).
func (s *AgentSession) SetBashRunning(running bool) {
	if s.control == nil {
		return
	}
	s.control.stateMu.Lock()
	s.control.bashActive = running
	s.control.stateMu.Unlock()
}

// AbortBash marks a running bash command as aborted.
func (s *AgentSession) AbortBash() {
	if s.control == nil {
		return
	}
	s.control.stateMu.Lock()
	s.control.bashAborted = true
	s.control.stateMu.Unlock()
}

// BashAborted reports whether bash was aborted.
func (s *AgentSession) BashAborted() bool {
	if s.control == nil {
		return false
	}
	s.control.stateMu.Lock()
	defer s.control.stateMu.Unlock()
	return s.control.bashAborted
}
